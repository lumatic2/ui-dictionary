...
- 캔버스 공급: `cd packages/component-registry && npm run build:catalog` 재실행으로 recipe 카탈로그가 재생성되어 Agent Design Insert 팔레트의 Recipes 섹션에 자동 반영된다 (FW milestone 배선, 2026-07-12). frontmatter 결손이 있으면 생성기가 비제로 exit로 크게 실패한다 — 배치 검증 체인에 이 명령을 포함한다.
...
- 2026-07-12: FW 배선 반영 — 검증 체인에 `build:catalog` 추가, 흡수 단계에 캔버스 Insert 팔레트 자동 공급 명시. 루프 산출은 이제 사이트·llms.txt·CLI·캔버스 4곳에 재작업 없이 도달한다.
