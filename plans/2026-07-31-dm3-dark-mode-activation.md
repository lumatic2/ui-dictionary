...
- **사람 관측 1회차 (2026-07-31): 랜딩 결함 3종 적발 → 수리** — ① `// hardcoded-color-ok` 마커가 JSX 자식 위치에서 **텍스트로 렌더**(DM2 위임 사고, 라이트에도 보이던 회귀) → 208줄 전부 제거, home-page.tsx 파일째 스캐너 allowlist ② 토큰 카드 위 데모 크롬(Commerce 스텝 라벨·Total·Image Treatment 라벨 등) slate 하드코딩 → 토큰화(에이전트 위임, 주석 추가 금지 지시) ③ 다크 쇼케이스 섹션·푸터가 DM2 인버스 토큰 치환으로 다크에서 희게 뒤집혀 내부 고정-다크 대시보드와 충돌 → 양 테마 다크 고정(slate-950/white) 복원. 자체 라이트 캔버스 데모(Physics·Design Canvas 등)는 의도된 라이트 아트보드로 유지 — 재관측 판정 대상.

## 진행 로그
- 2026-07-31 사용자 결정 갱신: **기본 테마 = 라이트 (OS 설정 무관)** — 구 결정(무선택 시 시스템 추종)을 대체. '시스템'은 명시 선택 시에만 저장·OS 추종. FOUC 스크립트·useSiteTheme 반영, OS-다크 에뮬레이션 검증 통과.
- 2026-07-31 작성.
