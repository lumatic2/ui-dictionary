...
- ~~품질 기준 = HTML 렌더 근접~~ — **사용자 관측 FAIL 로 뒤집힘 (2026-07-31, record 보존)**: "HTML 과 동일한 구조로 만들겠다는 목적 자체가 잘못" — HTML 덱이 여백 위주라 충실 재현할수록 성긴 PPT 가 된다.
- **품질 기준 v2 = brandlogy 편집 그래머 + askewly 토큰 대입** — 사용자 확정 2026-07-31 ("ㄱㄱ"). 기준 = `research/2026-07-31-pb2-brandlogy-grammar.md` + 캡처 2장(`research/sources/brandlogy-ppt-ref-*.png`). 프레임 고정 앵커(kicker·워드마크·주장형 헤드라인·근거 부제·페이지/출처 푸터) + 정보 3계층(주장→빅넘버→상세) + 색 규율(정량=키컬러·정성=먹색). 최종 게이트 = 사용자 관측 1회 유지.
- **콘텐츠 재구성 포함** — 사용자 확정 2026-07-31: 슬라이드당 주장 1+빅넘버 3~4+상세 2~3 이 나오게 PPT 용 구성을 재작성. 수치는 레포 실측만(발명 금지). slides.json(HTML 정본)은 무접촉 — PPT 구성은 덱 로컬 별도 레이어(`content/pptx-composition.json`)로, bespoke 코드가 소비 (매체별 구성 분리 = "매체가 다르면 게이트도 다르다" 정합. 튜닝값 결정).
- 대표 미세조정 슬라이드 선정(구도 복잡도 최상 1장) = 튜닝값(에이전트 결정, 장부 기록). v2 에서는 3계층이 다 들어가는 4번(자산) 채택.
- 시각 체크리스트 v2 (기준 교체에 따라): ① 프레임 앵커(kicker·워드마크·헤드라인·푸터) ② 3계층 밀도(하단 1/3 비지 않음) ③ 색 규율(단일 키컬러·정량/정성 구분) ④ 타이포 위계 4단 ⑤ 카드 문법(자기완결·헤어라인·radius).
