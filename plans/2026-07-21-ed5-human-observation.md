...
- **milestone**: ED5 — 계측이 못 잡는 것을 사람이 잡는다. 준비·관측·마감 3단계이고 통합검증은 사용자 과업 기록. milestone 규모.
...
- source-of-truth: 관측 기록 = `evidence/element-decision-layer/ed5-observation.md`.
...
- 판정 실패의 정당한 형태: 축이 안 갈려서 에이전트가 **사용자에게 한 줄 되묻는 것**은 실패가 아니다(ED3 프로토콜이 규정한 정상 경로). 실패는 근거 없이 조용히 고르거나, 틀린 요소로 구현까지 가는 것이다. 이 구분을 관측 전에 기록해둔다.
...
- [ ] **step-1 — 관측 준비**
  - Artifact: `evidence/element-decision-layer/ed5-observation.md` 스켈레톤 — 과업 3건 정의, 사전 설명 금지 프로토콜, 성공/실패 판정 기준(위 "정당한 실패" 구분 포함), 사용할 결정 자산 상태 스냅샷(군집 수·ED4 정답률)
  - Files: 읽기 `evidence/element-decision-layer/ed4-separation.md`·`docs/design-system/entry-protocol.md` / 쓰기 관측 기록 스켈레톤
  - Dependencies: none
...
- [ ] **step-2 — 사용자 관측 실행**
  ...
  - Files: 쓰기 `evidence/element-decision-layer/ed5-observation.md`
  - Dependencies: step-1
...
- [ ] **step-3 — 발견 결함 마감**
  ...
  - Dependencies: step-2
  - Verify: 수정 후 `python scripts/validate-decisions.py` exit 0 + ED4 케이스 재측정으로 회귀 0 확인
...
- **DoD**: 과업 3건의 성공/실패와 막힌 지점이 사용자 발화 인용과 함께 기록되고, 범위 내 결함이 마감되거나 미마감 사유가 명시되며, ED4 케이스 재측정에서 회귀가 0이다.
