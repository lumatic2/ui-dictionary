> **은퇴 2026-07-20 — 이 계획은 실행되지 않았다.**
> horizon 개설 시 작성·승인됐으나, 실행 세션이 이 문서의 존재를 확인하지 않고
> `plans/2026-07-20-th4-verification-materialization.md`를 새로 작성해 그쪽으로 실행했다.
> TH4의 정본 계약은 그 문서다. 이 문서는 계약이 두 개가 된 사실을 남기기 위해 보존한다.
> 실행된 계획은 이 문서의 DoD를 포함하고 넘어선다(입력 유래 단언은 여기 없던 것).
