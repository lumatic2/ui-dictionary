...
Status: closed — 회귀 (2026-07-31 사용자 관측: 랜딩 한국어 전환본 "너무 별론데, 되돌리자" → 영어 단일 확정. step-1 산출물은 영어 단일 정책 기록으로 개정 유지, step-2·3 은 폐기)
...
- 2026-07-31 step-2 사용자 중단 — 랜딩 한국어 전환본을 보고 "너무 별론데, 되돌리자" → 미커밋 변경 전량 원복(git checkout). 언어 방향 재결정 대기(설계 실패 계수: 계획 단계 결정이 실물 관측에서 뒤집힘 — ledger interruption 기록).
- 2026-07-31 재결정: **영어 유지로 회귀**(AskUserQuestion) — 정책 문서를 영어 단일 기록으로 개정, llms 설명 갱신, milestone 회귀 마감. 교훈: 언어·톤 같은 취향 결정은 문서 승인만으로 확정하지 말고 대표 표면 1곳 실물 시안으로 먼저 관측받는다.
