...
- [x] **step-2 — svg-filter-scene 레이아웃**
...
## finding 큐
- SVG 필터 기본 영역(110%)이 displacement 변위를 직선으로 잘라먹음 — 필터 영역 확장(x/y/width/height)으로 수리 (브라우저 실검증 적발). 레이아웃 문서화 대상 아님(렌더러 내부 처리).
