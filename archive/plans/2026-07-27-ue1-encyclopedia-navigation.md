...
## finding 큐
- O5 (관측 1회차): Get Started 착지 페이지가 비어 있다 — 사용자 "적어도 준비 중이라는 문구라도". 노출 정책("Coming soon 노출 금지, 게이트 뒤로")과 긴장 — 콘텐츠 채우기(UE3 인접) 또는 정책 예외 결정 필요.
- O6 (관측 1회차): Docs 랜딩("Getting set up")의 역할 불명 — 사용자 제안: Askewly Design 소개 또는 페이지 허브. Docs 랜딩 재설계 후보.
- O7 (관측 1회차): 좌측 내비 IA 뒤죽박죽 · 아코디언(기본 컴포넌트) 도달 경로가 사이드바에 없다 — UE2/UE3 계획의 1차 입력.
- O9 (관측 2회차): 검색 결과 화면 UI 품질 미달 (사용자 "너무 구려") — 재디자인 후보, askewly-design 경유.
- **결정 확정 (관측 2회차)**: 라우팅 분리 진행 — 사용자 "분리는 진행하는 게 좋겠어". UE5(pending)로 ROADMAP 등록, UE3 대량 콘텐츠 투입 전 실행 권고.
- O10 (관측 3회차): Header Sections 기존 예제 8종이 hero 와 구분 안 서고 몇 개는 서로 똑같아 보임 — 변형 차별성 미달. UE3 배치 1 의 1차 입력.
- **순서 확정 (관측 3회차)**: 다음 milestone = **UE5 (페이지 분리) 먼저** — 사용자 "그 다음 UE5 먼저 할거고".
