...
- `examples/ui-vocabulary-site/src/App.tsx` - 왜: Card Headings leaf examples와 preview renderer가 구현된 파일이다.
- `examples/ui-vocabulary-site/src/lib/navigation-model.ts` - 왜: `plus-application-headings-card-headings` leaf의 실제 IA 위치를 확인한다.
- `docs/research/tailwind-plus-catalog-interactivity-ledger.md` - 왜: 이전 Heading interactivity sweep과 검증 기록을 이어받는다.

## 작업
- Tailwind Plus Card Headings 원본 페이지와 로컬 Card Headings leaf 페이지를 각각 캡처한다.
- 캡처를 비교해 Card Headings leaf에서 부족한 구조, 상호작용, motion, light/dark behavior를 보강한다.
- 작업 범위는 `plus-application-headings-card-headings` leaf와 해당 card heading preview variants로 제한한다.
...
2. 로컬 Card Headings leaf에서 create/contact/dropdown/menu/row controls, light/dark toggle을 직접 클릭한다.
...
## 금지사항
- Card Headings leaf 외의 pages를 이 step에서 함께 수정하지 않는다. 이유: 작업 단위를 한 페이지로 고정한다.
