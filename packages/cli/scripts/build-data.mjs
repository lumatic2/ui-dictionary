...
async function main() {
  ...
  const designMd = await readFile(path.join(repoRoot, "DESIGN.md"), "utf8")

  const siteIndexCss = await readFile(
    path.join(repoRoot, "examples", "ui-vocabulary-site", "src", "index.css"),
    "utf8"
  )
  const themeMatch = siteIndexCss.match(/@theme inline \{[\s\S]*?\n\}/)
  if (!themeMatch) {
    throw new Error("could not extract the @theme inline block from the site index.css")
  }
  const askewlyCss = [
    "/* GENERATED by @askewly/design — imports the token custom properties and binds",
    "   them to Tailwind v4 utilities. Usage in your global stylesheet:",
    '   @import "tailwindcss"; @import "./askewly.css"; */',
    '@import "./tokens.css";',
    "",
  ...
  await writeFile(path.join(dataDir, "askewly.css"), askewlyCss)
  await writeFile(path.join(dataDir, "DESIGN.md"), designMd)
