...
export function renderBrandCss(answers: BriefAnswers): string {
  ...
  const font = FONT_STACKS[answers.type]
  // `color-scheme` is what the *browser* reads. Tokens only repaint what we
  // draw; scrollbars, native selects and checkboxes, and the overscroll gutter
  // behind the page are the browser's, and they stay light without this.
  const vars = (m: { bg: string; fg: string; card: string; muted: string; mutedFg: string; border: string }, primary: string, onPrimary: string, ring: string, scheme: string) => `  color-scheme: ${scheme};
  --background: ${m.bg};
  --foreground: ${m.fg};
  --card: ${m.card};
...
/* One switch for dark mode. Without this line Tailwind v4 keys \`dark:\`
 * utilities off \`prefers-color-scheme\` while the token values below key off
 * the \`.dark\` class — two independent switches, so a project that toggles the
 * class gets dark tokens with light-mode utilities (and an OS-dark visitor gets
 * the reverse). Transplanted shadcn primitives carry \`dark:\` utilities, so this
 * is load-bearing, not cosmetic. Toggle by putting \`dark\` on <html>. */
...
.dark {
${vars(c.dark, a.dark, a.onDark, a.darkRing, "dark")}
}
...
export async function runKickstart(
  ...
  steps.push(
    `import "./askewly-brand.css" after \`@import "tailwindcss";\` in your global stylesheet (order matters — it registers the dark variant)`,
  )
  steps.push(`toggle dark mode by putting the \`dark\` class on <html> — tokens and \`dark:\` utilities both key off it`)
  steps.push(
    exportName
