...
- [ ] Step 1 — Palettes 실데이터 + 상호작용: 큐레이션 팔레트 라이브러리 데이터(PGD1 seed 라이브러리 재사용+확장), 팔레트 카드 렌더(이름·스와치·hex), 복사(hex/전체)·내보내기 실동작, 라이트/다크 대응. (verify: dev 렌더 + 복사/내보내기 동작 + lint/build)
- [ ] Step 2 — Colors 축 공개 + 배포: 완성 판정 체크리스트 통과 확인 후 Colors 축(Generator+Palettes)의 shell 게이트를 열어 프로덕션 노출(헤더 nav "Colors" 공개), Chrome evidence(데스크톱/모바일·라이트/다크), push 승인 후 배포·실사이트 확인. (verify: 완성 판정 3항목 체크 + 프로덕션 스모크; 실패 모드 — 다른 껍데기[Foundations/Pro 하위/Download]는 여전히 비노출 확인)
