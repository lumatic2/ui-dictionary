# Roadmap Gap Review

Date: 2026-07-12

## North Star
Goal: 일회성 레퍼런스 수집을 반복 가능한 흡수 루프(수집→중복 필터→적응→검증→흡수)로 운영화하고, 전 제품 표면으로 확장하며, 산출이 캔버스 레지스트리·CLI·llms.txt에 재작업 없이 공급되게 한다. (상세 plan → `docs/horizons/2026-07-living-design-system.md`)

## Current State
- RL: RL — Reference Loop Pipeline (evidence: docs/research/loop/ledger.md (3 batch rows) + changesets/README.md rows 64-68)
- MS: MS — Mobile Surface Batch (evidence: docs/research/loop/ledger.md rows 4-5 + changesets/README.md rows 69-72 + docs/research/assets/ms-step4-mobile-viewport-e2e.png)
- FW: FW — Feed Wiring (evidence: changesets/README.md rows 73-76 + docs/research/assets/fw-step3-recipe-insert-e2e.png + packages/component-registry/src/recipe-catalog.generated.ts)
- SD: SD — Surface Depth 2세대 (구 CS+R2 병합, 2026-07-12 사용자 확정) (evidence: docs/research/loop/ledger.md rows 6-9 + changesets/README.md rows 77-82 + docs/design-system/anti-patterns.md + docs/design-system/token-coverage-gen2.md)

## Gap
- Active harness milestones are exhausted.
- Compare the north star above with current evidence before starting new implementation.
- Do not infer completion without a new DoD and evidence path.

## Proposed Next Horizon
- N1 - define the next measurable gap.
- N2 - create one evidence-producing milestone.
- N3 - add the smallest validation or smoke gate.

## Recommendation
Promote one proposed item to ROADMAP.md only after the user approves the next horizon.
