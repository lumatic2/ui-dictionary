...
- 실행: `node scripts/audit-recipe-candidates.mjs` (Step 2에서 신설 예정 — 이 문서는 그 도구의 계약을 전제로 절차를 서술한다. 기존 `scripts/audit-ui-vocabulary-candidates.mjs`와 동일한 역할을 recipes/`terms.yml` 양쪽 대비로 수행한다).
...
- 신규 recipe가 있으면 `scripts/generate-llms-txt.mjs`의 `FIXED_ASSETS`에 수동 추가한다 (§llms.txt 등록 참조).
...
`scripts/generate-llms-txt.mjs`는 현재 `FIXED_ASSETS` 목록 기반으로 동작한다 (신규 recipe/asset 자동 발견 기능 없음). RL 범위에서는:

- 신규 recipe가 승격되면 `generate-llms-txt.mjs`의 `FIXED_ASSETS` 배열에 **수동으로** 경로를 추가한다.
- 자동 발견(디렉터리 스캔 기반 registry 배선)은 이 milestone의 범위 밖이며 FW milestone으로 유보한다 (계획서 Scope 참조).
