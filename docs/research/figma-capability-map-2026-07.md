...
1. 세션 재시작 후 원격 Figma MCP 도구 스키마 실측 (읽기/쓰기 범위) — 테스트 파일로 실호출
2. 계정 플랜 확정 (Starter 여부) → 로컬 Dev Mode MCP 경로 재판정
3. FB3 variables 쓰기 경로: 원격 MCP 쓰기 범위 실측 후, 부족하면 변수 import 플러그인 선정 또는 자체 플러그인 제작 판단
