...
| # | 심각도 | 표면 | 증상 | 라우팅 |
|---|--------|------|------|--------|
| 1 | major | 캔버스 삽입 | 팔레트 삽입(primitive·registry·recipe) 전부 "unsupported canvas operation" 거부. 원인: `apps/agent-design-desktop/src/contract.ts:249` operation 화이트리스트에 UX2 `batch` 타입 누락 — dev 브라우저(브리지 없음)에서만 검증돼 갭이 숨어 있었음 | **QA2 Step 6** (packaged E2E registry 조립 시나리오 통합 시 계약 확장 필수) |
