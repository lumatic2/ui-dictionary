...
axes:
  - id: option-count
    ...
    confidence: high
  - id: option-set-known
    question: "보기 목록이 미리 정해져 있는가, 데이터에서 오는가"
    values: ["predefined", "open-ended"]
...
rules:
  ...
  - when: { option-set-known: "open-ended" }
    pick: combobox
    because: "목록이 미리 정해지지 않고 데이터에서 오면 고정 목록으로 표현할 수 없다 (Carbon 명시)"
...
**"많은 선택지" 군집을 여기에 흡수했다.** 별도 군집으로 만들려던 combobox/select/radio 판정은 이 군집의 `option-count: 20+`과 `option-set-known: open-ended` 두 축이 덮는다. 후자의 근거:

> "Use a combo box when the user needs to select one option, but the list of options may be very long or not predefined. ... Combo boxes are useful when the data populating the list may come from a database." — IBM Carbon, Dropdown usage
