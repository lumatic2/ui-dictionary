...
| `marketing-landing` | Standard-funnel landing page (one page component + nine section files + `data.json` copy/mock surface): hero → logo strip → story → demo proof → comparison → pricing → FAQ → final CTA → footer. Composes assets: `floating-bars-hero`, `logo-marquee`, `zigzag-story-section`, `terminal-demo-panel`, `staggered-entrance-group`, `contrast-duo-card`, `mesh-gradient-surface`, `rotating-label`, `scroll-driven-reveal`. Pricing/FAQ/CTA/footer are Askewly originals (external candidates rejected — Next.js coupling/paid walls, see `research/2026-08-04-m27-marketing-block-absorption-survey.md`). **Rearrangement guide**: section order *is* the funnel type (`research/2026-07-19-st4-composition-patterns.md`) — move the proof sections (logo strip, comparison) directly under the hero for skeptical-market "proof-first" funnels, or keep only hero + single CTA for compressed Z-pattern lead-gen; each section is one import line in `page.tsx`. | Askewly original composition (no absorbed external source). | Standard shadcn set (`--background --foreground --card --card-foreground --popover --popover-foreground --primary --primary-foreground --secondary --secondary-foreground --muted --muted-foreground --accent --accent-foreground --destructive --destructive-foreground --border --input --ring --radius`), defined by the consuming project's token layer. |

| `docs-site` | Documentation site skeleton (one page component + navbar + three page files + `data.json`): thin top navbar with section tabs and a ⌘K command palette, a left rail whose version dropdown swaps the sidebar tree (stale-version banner included), and three pages — guide article (package-manager code tabs, self-playing terminal proof, related-reading grid), HTTP API reference (nested parameter rows, per-language samples), changelog (category filter, pagination). Composes assets: `versioned-docs-switcher-navbar-sidebar-swap`, `doc-search-cmdk-grouped-results-panel`, `docs-code-block`, `terminal-demo-panel`, `responsive-content-grid`, `api-reference-layout`, `docs-changelog-category-filter-page`. **State split**: filter, pagination and version state live in the block, because what "matches a category" depends on the consumer's tagging scheme — the assets take already-narrowed data. Navbar, article prose, on-page TOC and mock content are Askewly originals. | Askewly original composition (no absorbed external source). Every open docs framework surveyed — Fumadocs, Nextra, Starlight, Docusaurus, VitePress — owns its own router and build pipeline, so none can be copied in as plain React files; the two MIT candidates that could (shadcn/ui `apps/www`, `@mintlify/components`) do not carry a docs shell. See `research/2026-08-05-m29-docs-site-absorption-survey.md`. | Standard shadcn set (`--background --foreground --card --card-foreground --popover --popover-foreground --primary --primary-foreground --secondary --secondary-foreground --muted --muted-foreground --accent --accent-foreground --destructive --destructive-foreground --border --input --ring --radius`), defined by the consuming project's token layer. No sidebar or chart tokens — the left rail is composed markup, not the shadcn `sidebar` primitive. |
...
- 2026-08-06: M29 — `docs-site` 블록 §6 등재 (자체 조합 — 흡수 후보 8건 전수 기각, 상태 분리 계약 명시).
- 2026-08-04: M27 — `marketing-landing` 블록 §6 등재 (자체 조합 — 재배열 가이드 포함).
