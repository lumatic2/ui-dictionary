...
> Status: Studio Finish 제안(사용자 승인 대기) — cascade-studio 이월 갭 3건 마감
> North star: Build Askewly Design as both a public reference website and an agent-usable implementation system.
> line budget: <=150

## Current Horizon

<!-- harness:goal id="studio-finish" status="active" -->
...
<!-- harness:milestone id="SF1" status="completed" priority="P0" evidence="evidence/studio-finish/sf1-injection-e2e.md, changesets/20260719-studio-data-injection/README.md, changesets/20260719-injection-contract/README.md" -->
### SF1 — 데이터 주도 스튜디오 주입 (자동화)
- DoD: 데이터 JSON + `make-studio.py` 1커맨드로 스튜디오 HTML 생성 실구동(기본·커스텀 각 1) + 불량 JSON 명시 에러 + 계약 배포(구 지시 grep 0).
- Evidence: evidence/studio-finish/sf1-injection-e2e.md
- Gap: 후보 주입이 수동 python 편집 — 매 의뢰 오류·비일관의 근원 (ST4 이월)
- Scale: changesets>=2; surfaces: 로컬 스튜디오 실구동, ui.askewly.com curl; capability: 반복 가능한 데이터 주도 주입
- Status: [x]

- Completed at: 2026-07-19
- Summary: 데이터 JSON + make-studio.py 1커맨드 주입 실증 — 실패 경로 포함
<!-- harness:milestone id="SF2" status="completed" priority="P0" evidence="evidence/studio-finish/sf2-composition-e2e.md, changesets/20260719-composition-types/README.md, changesets/20260719-booking-patterns/README.md" -->
### SF2 — 구성 패턴 완편 (4유형 + 예약형)
- DoD: 구성 4유형 미리보기 반영 전수 실측 + 예약형 절 llms 배포 + 조건 노출 경계 확인.
...
- Gap: 리서치 12유형 중 4종 미편입 + 예약형 특수 패턴 미반영 (ST4 이월)
- Scale: changesets>=2; surfaces: 로컬 스튜디오, ui.askewly.com curl; capability: 리서치 전량 편입된 구성 축
- Status: [x]

- Completed at: 2026-07-19
- Summary: 구성 12유형+예약형 편입, 조건 노출 경계 실측
<!-- harness:milestone id="SF3" status="active" priority="P1" -->
### SF3 — 미리보기 고도화 (다크·반응형)
- DoD: 다크·뷰포트 토글 실구동 + 조합 매트릭스 12조합 실측 + 커스텀 데이터 전 루프 1회(horizon 통합 검증).
- Evidence: `plans/2026-07-19-sf3-preview-upgrade.md`, `evidence/studio-finish/`
- Gap: 다크/접근성 축 선택을 미리보기로 확인 불가 — 실물 원칙의 잔여 구멍 (ST4 이월)
- Scale: changesets>=2; surfaces: 로컬 스튜디오 E2E; capability: 다크·모바일까지 실물 미리보기
- Status: [ ]
