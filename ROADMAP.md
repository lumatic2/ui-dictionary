...
> Status: 템플릿 제작 시스템 경화 — TH1~TH5·TH7·TH9~TH11 완료, TH6 실연 통과(결함 3건) → TH12(자기완결 산출물) → TH6 close. 편집기 UI는 별도 horizon
...
<!-- harness:milestone id="TH11" status="completed" priority="P1" -->
...
- Status: [x]
- Completed at: 2026-07-20
- Summary: 청사진이 규격을 선언하고 px가 mm에서 파생된다(종횡비 추론은 두 규격이 겹쳐 답이 없었다). A계열 4종·인쇄판 2종·도련 포함 SVG + 재단 표시 8선. changesets 185/186/187

<!-- harness:milestone id="TH12" status="completed" priority="P0" -->
### TH12 — 자기완결 산출물
- DoD: 내보낸 파일 하나를 옮겨도 그림이 함께 간다 + 게이트가 외부 참조를 막는다 + TH6 명함이 그림 있는 채로 렌더된다.
- Evidence: evidence/template-production-hardening/th12-self-contained.md
- Gap: 소재 URI가 파일 경로라 SVG를 <img>로 끼우면 그림 없이 렌더된다 (TH6 실연 결함 1 — 발주 차단 가능)
- Note: 사용자 확정 2026-07-20 — 편집기 UI 재설계보다 먼저. horizon close 전에 닫는다
- Scale: changesets>=2; surfaces: 소재 공급자·게이트·서명; capability: 한 파일로 발주 가능한 산출물
- Status: [x]
- Completed at: 2026-07-20
- Summary: 소재가 문서에 실린다(data URI). 같은 <img> 렌더 경로가 전에는 그림 없이, 지금은 그림 있게 나온다. changesets 189/190

<!-- harness:milestone id="TH6" status="active" priority="P1" -->
