...
<!-- harness:milestone id="SD" status="active" priority="P0" -->
### SD — Surface Depth 2세대 (구 CS+R2 병합, 2026-07-12 사용자 확정)
- DoD: 4개 표면(commerce/internal-tools/documentation/mobile-apps) 각각 심화 배치 1회 완주(ledger 4행, 표면당 recipe +2 이상, 총 recipe 31+, 각 배치 검증 체인+build:catalog PASS), `docs/design-system/anti-patterns.md`가 FIXED_ASSETS 등록으로 llms.txt에 노출, 토큰 갭 리포트 + 필요 확장분 SSOT 반영(lint-tokens PASS).
- Evidence: `docs/plans/2026-07-12-sd-surface-depth.md`, `docs/research/loop/ledger.md` 추가 4행, changesets, `docs/design-system/anti-patterns.md`
- Gap: RL/MS가 연 표면의 recipe 깊이가 얕고(2~4종), anti-pattern 지식이 recipe에 파편화돼 agent 진입점에서 통합 소비 불가, 신규 표면의 토큰 갭 미점검.
- Status: [ ]

## Next Candidates (같은 horizon)

- 없음 — SD 완료 시 H2 닫는 기준 충족 여부를 close 판정한다.
