...
> Status: 템플릿 제작 시스템 경화 — TH1~TH3 완료, TH7(렌더 충실도) 진행 중 → TH4 → TH5 → TH6
...
- Completed at: 2026-07-20
<!-- harness:milestone id="TH7" status="active" priority="P0" -->
### TH7 — 캔버스 렌더 충실도
- DoD: 편집기와 내보낸 SVG/HTML 양쪽에서 템플릿이 토큰 색·글꼴·타이포·이미지를 갖고 나타나고, 토큰 세트 변경 시 셋 다 함께 바뀜 + 해석 실패는 진단으로 드러남.
- Evidence: evidence/template-production-hardening/th7-render-fidelity.md
...
- Blocked by: TH7 (충실도 전에 게이트를 세우면 색 빠진 산출물을 통과로 고정한다)
