    ...
    sections: [
      {
        title: "Tokens",
        body: [
          "primitive tier는 원시값만 담습니다: 12단계 violet-tinted gray ramp(`color.primitive.gray.1`–`12`), `color.primitive.white`, 브랜드 세트(`askewly.violet`, `violet-deep`, `orchid`, `lavender`, `sky`, `mint`), destructive 상태용 `color.primitive.red.9`, 그리고 강조·상태색 램프(`indigo`, `skyx`, `emerald`, `rose` — 사이트가 실제로 렌더하던 shade만 등재)입니다. gray ramp에는 토큰 소스에 명시된 사용 관례가 있습니다 — 1-2는 앱 배경, 3-5는 interactive 컴포넌트 상태, 6-8은 border, 9-10은 solid/low-contrast foreground, 11-12는 text입니다.",
          "semantic tier는 컴포넌트가 실제로 참조하는 계층입니다: `surface.base/raised/overlay/muted/secondary/tint`, `text.default/muted/secondary/on-accent/on-destructive`, `action.primary/primary-hover/secondary/destructive`, `accent.base/foreground`(mint 계 인터랙션 워시), `emphasis.surface/foreground/solid/solid-hover/ring/on-solid`(indigo 계 선택·강조 — accent와 역할이 다릅니다), `status.info-*/success-*/danger-*`(sky·emerald·rose 계 상태색), `border.default/input/focus/accent`. 각 semantic 토큰은 기본적으로 primitive 하나를 참조하고, 별도의 다크 모드 값을 가질 수 있습니다.",
