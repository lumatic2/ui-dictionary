...
4. **Close the loop with verification — and the gate depends on the medium you decided in N-0.** Checking a print piece with the screen checklist passes work that is broken paper. Fetch [docs/design-system/medium-taxonomy.md](https://ui.askewly.com/llms/docs/design-system/medium-taxonomy.md) if you need the reasoning; the gate itself is:
   ...
   - **Screen** — interaction states complete (hover/focus/active/disabled/loading/empty/error), dark mode holds, WCAG contrast passes, responsive boundaries do not collapse. The website's [Getting set up page](https://ui.askewly.com/docs/getting-started-setup) describes the same loop for humans: Explore → Acquire → Inject → Verify (human canon for install/usage commands; repo-side summary lives in `README.md` §How to use).
...
4.5. **Run the machine check, then fix what it finds — before a human ever sees it.** `npx @askewly/design verify <dir>` scans for color values that bypass the token system (hex literals, raw `rgb()`/`hsl()`) and counts how many distinct type sizes one file uses. **If it reports violations, fix them and run it again.** Attaching the report and moving on defeats the purpose: this step exists so you correct yourself, not so the user gets a list of your defects. It is a **warning** today, not a blocker — if you deliberately leave a violation, say which and why in the report. (Options: `--ext tsx,css` to narrow, `--typography-threshold <n>` when a project's scale genuinely differs.)
...
0. **Block-first for skeleton-scale asks.** If the request is a whole product surface rather than one screen — an app shell, a dashboard-plus-pages skeleton, a hackathon/new-product kickoff — check [docs/design-system/block-contract.md](https://ui.askewly.com/llms/docs/design-system/block-contract.md) §6 for a matching block before composing from individual recipes or assets. A matching block is the default starting point (transplant → mandatory restyle → verify, per the contract's consumption path); compose by hand only when no block fits, and note the gap in the report. **For a brand-new project starting from a block, use the kickstart path (block-contract §8)**: ask the user the three abbreviated-brief questions (tone, color direction, type direction — options + a recommendation each, per design-brief's 축약 모드 row), then run `npx @askewly/design init --block <name> --yes --tone <t> --color <c> --type <ty>` and continue from its verify output. The command generates the project's DESIGN.md — the brief answers land there, not in a side note.
