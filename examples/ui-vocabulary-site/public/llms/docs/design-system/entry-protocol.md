...
1. Fetch [tokens/askewly.tokens.json](https://ui.askewly.com/llms/tokens/askewly.tokens.json) — every color, dimension, and type value must derive from semantic tokens. Never hard-code literals.
...
- Semantic tokens only; never primitive-tier references or raw literals in component code.
