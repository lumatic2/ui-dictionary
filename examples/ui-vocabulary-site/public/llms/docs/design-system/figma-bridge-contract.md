...
| 순위 | 채널 | 용도 |
|---|---|---|
| **주력** | 원격 Figma MCP `use_figma` (Plugin API JS) | variables 생성·바인딩·읽기 전부 실측 검증됨. 호출 전 `skill://figma/figma-use/SKILL.md` 로드 필수 |
...
**현행화 (2026-08-01 재가동 실측, `research/2026-08-01-m14-figma-channel-recheck.md`)**: 기기에 MCP 미등록이면 `claude mcp add --transport http --scope user figma https://mcp.figma.com/mcp` 재등록 — 인증은 세션 내 `/mcp` OAuth 만 가능(CLI `auth` 없음, human gate). 도구 26종은 인증 직후 **세션 재시작 없이** 노출된다(FB1 시절과 달라진 점). 대형 반환은 ~20kb 절단 — 청크·지문 회수로 우회(방법론 ③).
...
돌아오는 방향은 성격이 다른 두 lane 으로 갈린다. 혼동하면 역수입 금지 원칙이 무너진다.

### 3a. 사람 디테일링 회수 lane (왕복의 닫는 절반 — 2026-08-02 M14 정본화)

- 대상: **우리가 승격한 프레임**에서 사람이 다듬은 변경. 절차는 기계화돼 있다 — 승격 시 push 스냅숏 장부(`scripts/figma-push-snapshot.mjs`) → 디테일링 후 재캡처 → `scripts/figma-return-diff.mjs` 전수 대조(charCode 스캔 포함) → **사람이 바꾼 것만** 코드에 반영 → 브라우저 재검증. 절차 정본: `methodology/figma-workflow.md` §2③⑤.
- 이 lane 도 토큰은 역수입하지 않는다 — diff 가 토큰 값 변경 욕구를 드러내면 SSOT 수정 → §2 재동기화로 돌린다.
- 실증: FW2-1(변경 회수 — 152→128px·U+2028 줄바꿈) + M14(무변경 왕복 0/0/0, 오검출 0).

### 3b. 레퍼런스 흡수 lane (선별적)

- Figma 자산(프레임·컴포넌트·외부 UI kit)은 **레퍼런스 입력**이다: 스크린샷 + 구조 노트로 `research/`에 캡처하고, 채택할 패턴만 근거와 함께 `knowledge/`·`tokens/`로 승격한다 (기존 레퍼런스 흡수 루프와 동일 절차).
...
- 2026-08-02: M15 — §1 채널 현행화(재등록·/mcp 인증·재시작 불필요·20kb 절단), §3 을 두 lane(3a 사람 디테일링 회수=기계화 정본 / 3b 레퍼런스 흡수)으로 분리 명문화. 근거: M14 왕복 2회차(`evidence/figma-return-path/m14-roundtrip-2.md`).
