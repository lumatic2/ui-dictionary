...
const FIXED_ASSETS = [
  ...
  },
  {
    ...
    items: [
      ...
      ["docs/design-system/copy-language.md", "Copy language policy: site copy is English-only (user-confirmed 2026-07-31 after a Korean-conversion attempt was reverted on observation) — do not re-propose Korean conversion; existing Korean strings stay as-is"],
    ],
  },
  {
    section: "Tokens",
  ...
  },
  {
    ...
    items: [
      ...
      ["docs/design-system/block-contract.md", "Block contract: page/app-skeleton-grade composed assets — tier marker (meta.tier), registry representation, layout contract, required-CSS-variable restyle obligation, absorption/attribution rules, and the transplant→restyle→verify consumption path"],
      ["docs/design-system/head-meta.md", "Head meta contract: minimum OG/favicon/tab-title obligations for every deployed project — static OG fallback (SPA crawlers never see client-side updates), dual favicon (ico+svg), per-route title override; snippet at templates/head-meta.html.tmpl"],
      ["docs/design-system/harvest-contract.md", "Harvest contract (input loop): how self-produced deployed surfaces get promoted back into the registry — candidate axes + user-taste gate, standard 6-step promotion (extract, restyle-normalize, requiredCssVars, registry, verification chain, user observation), attribution and dedup rules"],
      ["docs/design-system/figma-bridge-contract.md", "Figma bridge contract: token SSOT wins (Figma variables are a derived artifact), outgoing sync rules (idempotent upsert, name-preserving, own-collection-only deletes), and the two return lanes — mechanical human-detailing retrieval (push snapshot ledger + diff) vs selective reference absorption"],
