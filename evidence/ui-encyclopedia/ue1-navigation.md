...
## 사람 관측 2회차 (2026-07-27) — 부분 통과 + 신규 결함 2건

발화 원문:

> "검색 추천에는 있는데 검색 결과로는 안 나오네. 뒤로가기는 되고. 헤더는 추천에는 나오는데 검색하면 결과로 안 나옴. 결과 창 UI도 개선해야겠네. 지금은 너무 구려."
> "한 페이지에 다 모여 있는 구조, 분리는 진행하는 게 좋겠어"

- **O1(뒤로가기) 통과 확인.**
- **O8 (신규, 좁은 수리)**: 검색 *결과 리스트*가 여전히 필터에 갇혀 "0개 결과" — 제안(O2)과 결과(`searchTerms`)가 별도 경로였고 결과 쪽을 안 고쳤다. 스크린샷 증거: 추천엔 아코디언, 결과는 0건. **수리** — `searchTerms` 를 "검색어 있으면 전체 사전, 동점이면 필터 안 우선"으로 교정.
- **O9 (신규, 구조)**: 검색 결과 화면 UI 품질 미달 ("너무 구려") — finding 큐.
- **결정**: 라우팅 분리 진행 확정 (사용자) — UE5 후보로 등록.

### O8 수리 검증 (실행 관측)

```
아코디언 PASS zero=False hit=True
헤더 PASS zero=False hit=True
accordion PASS zero=False hit=True
docs-scope 아코디언 PASS
CONSOLE ERRORS: 0 · O8 FIX: PASS
```

회귀 게이트: `npm run build` exit 0 · `npm run lint` exit 0.

## 사람 관측 3회차 (2026-07-27) — **통과**

- 검색 축: "ㅇㅇ 아코디언 나온다" — 검색 결과 리스트에서 아코디언 확인. 연관 용어(확장 상태 ARIA·펼침 버튼·패싯 필터)가 함께 나오는 이유를 물어 전문 검색의 "설명 일치" 동작으로 확인 — 결함 아님, 단 정확 일치와 연관 언급의 계층화는 O9 재디자인에 포함.
- 사이드바 축: "'Header Sections' 눌렀을 때 예제 8가지 있는거 봤어" — 목록 도달 확인.
- **신규 발견 O10 (콘텐츠 품질, UE3 입력)**: "근데 이게 헤더가 맞아? hero 아님? 그리고 몇 개는 그냥 똑같이 생겼는데?" — Header Sections 기존 예제가 hero 와 구분이 안 서고 변형 간 차별성이 부족하다. Tailwind 분류상 Header Section=페이지 머리말 섹션(내비바는 Elements>Headers)이라는 분류 설명과 별개로, 변형 차별성 미달은 실결함.

**판정: DoD 3항(검색 도달·사이드바 도달·딥링크 재현) 충족 — 관측 3회 왕복으로 확인. UE1 닫는다.**
