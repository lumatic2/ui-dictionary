...
```
용어 원본            562
배포된 용어          0  ← 사전이 에이전트 경로에 없다
레시피               45 (참조 보유 35)
distinct term_refs   81
끊긴 term_refs       91  ← 배포된 계약 안의 죽은 이름
용어 커버리지        14.4%  (자산 없는 용어 481)
코드 자산            27
프로토콜 용어 언급   0
끊긴 참조 사유별: 로컬에만 존재 — 미배포: 91
```

수치 해설:
- **81 vs 91** — 서로 다른 용어 이름은 81개, 그 이름이 레시피에 등장하는 **횟수**는 91회다. 앞선 수기 계수는 distinct만 셌다. 고쳐야 할 자리는 91곳.
- **끊긴 사유가 전부 "로컬에만 존재"** — 참조가 오타이거나 없는 용어를 가리키는 게 아니다. 이름은 전부 정확한데 **배포가 안 됐을 뿐**이다. VL2가 배포하면 대부분이 자동 해소된다.
- **자산 없는 용어 481개** — 판정이 정확해져도 이 용어를 고르면 만들 근거가 없다. VL3 폴백 규약의 근거.

### 실패 probe (계수기가 배포본 기준으로 세는가)

배포 디렉터리에 `cart-summary` 1개만 담은 fixture를 넣고 재실행:

| 상태 | 배포된 용어 | 끊긴 term_refs |
|---|---|---|
| fixture 없음 | 0 | **91** |
| fixture 투입(`cart-summary`) | 1 | **88** |
| fixture 제거 | 0 | **91** |
