...
- 관측 최종 2026-07-31: **PASS** — 관측 왕복 4회(FAIL→기준 교체→f4 그림자·카드 지적→f5 구도·아이콘 질문→"괜찮긴 한데"+이미지 트랙 탐색) 끝에 **3트랙 구도(네이티브 정본/하이브리드/풀-이미지+사실 검수)로 사용자 확정("그래 ㄱㄱ")**. 흡수분 custom-skills `references/pptx-bespoke.md` §5~8 반영·배포(커밋 22cc7f2).
- codex 위임 실증 2본: `decks/claude-ppt-lab/codex-image-deck/`(하이브리드 — 생성 히어로+네이티브 본문, image_gen 프롬프트 템플릿 획득) · `codex-fullimage-deck/`(풀-이미지 — 비주얼 상한 최고, 단 콘텐츠 왜곡·오타·폰트 이탈·편집 불가 실측 → 사실 검수 게이트 근거).
