...
- 사용자 관측 4회차 2026-07-31: **부분 FAIL — 1건 반영** (깜빡임 해소 확인, 단 본편-스피커 타이밍 어긋남) 원인 = 재연결 지연: 장 넘김마다 본편이 새 문서로 교체되고 다음 하트비트(≤1.2s)가 와야 스피커가 따라옴 → `navigateTo()` 목적지 선공지 + `__navigating` 레이스 가드. 실측 지연 42/269/21ms·역방향 24ms(`SYNC-LAG PASS`), 회귀 4종 PASS. custom-skills 205810b. **재관측 대기.**
