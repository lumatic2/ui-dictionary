...
| 209 | 20260721-token-validation-all-paths | 2026-07-21 | ECT1 follow-up after an independent verifier refuted the completion claim: only set-node-property was gated, while update-node's patch.tokenBindings wrote through Object.assign with no checks — and that is the live-bridge surface agents use, i.e. exactly the threat model the milestone claimed to close. The rule is now a single validateTokenBinding both paths call, pinned by a test asserting both return the same reason string | 6/6 | complete |
