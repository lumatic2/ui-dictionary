...
- **실행이 잡은 결함 1건 (E2E 게이트의 값)**: 1차 스니펫이 `"baseUrl": "."` 를 포함했는데 **TypeScript 6.0.3 에서 `TS5101` 로 빌드가 깨진다**(deprecated). 미싱 모듈 6건을 deprecation 1건으로 바꾼 셈. `paths` 는 **TS 4.1** 부터 baseUrl 없이 tsconfig 상대로 해석되므로 `baseUrl` 을 뺐다(초안에 4.4 로 적었던 것을 2026-08-06 공식 문서 대조로 정정 — `https://www.typescriptlang.org/tsconfig/#paths`). 테스트에 `not.toContain("baseUrl")` 로 고정.
