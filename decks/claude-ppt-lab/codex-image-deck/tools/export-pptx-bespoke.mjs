...
const { THEME_ROOTS } = await import(new URL('../../tools/src/theme.mjs', import.meta.url)); // claude-ppt-lab 공용 theme (askewly 포함)
...
for (const s of comp.slides) {
  ...
  if (s.kind === 'cover') {
    if (s.heroImage) {
      // 생성 이미지는 표지의 오른쪽 장면으로만 사용한다. 나머지 정보와 편집은 네이티브 PPTX 요소다.
      slide.addImage({ path: path.join(root, s.heroImage), x: 7.1, y: 0.68, w: 5.48, h: 6.14, transparency: 6 });
      slide.addShape('rect', { x: 6.35, y: 0.68, w: 0.9, h: 6.14, fill: { color: BG, transparency: 10 }, line: { color: BG, transparency: 100 } });
    }
    frame(slide, { pageNo: s.no });
    const coverSize = s.title.length > 14 ? 35 : 48;
    slide.addText(s.title, { x: MX, y: 2.18, w: 6.2, h: 1.46, fontFace: FONT, fontSize: coverSize, bold: true, color: INK, breakLine: false, fit: 'shrink', charSpacing: -0.5, margin: 0 });
    slide.addText(s.subtitle || '', { x: MX, y: 3.85, w: 5.5, h: 0.55, fontFace: FONT, fontSize: 14, color: SUB, margin: 0 });
    const chips = s.chips || [];
    const chipW = 1.0, gap = 0.2, totalW = chips.length * chipW + (chips.length - 1) * gap;
    chips.forEach((label, i) => {
      const cx = MX + i * (chipW + gap);
      slide.addShape('roundRect', { x: cx, y: 4.68, w: chipW, h: 0.36, rectRadius: 0.18, fill: { color: RAISED }, line: { color: BORDER, width: 1 } });
      slide.addText(label, { x: cx, y: 4.68, w: chipW, h: 0.36, fontFace: FONT, fontSize: 10.5, color: SUB, align: 'center', valign: 'middle', margin: 0 });
    });
    continue;
  }
