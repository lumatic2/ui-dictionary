export function createShellRenderers({
  ...
  if (!document.documentElement.classList.contains('print-mode') && !document.documentElement.classList.contains('capture-mode')) {
    ...
    // 목적지 선공지 + 이탈 중 플래그 — 이탈 중 hello 응답이 옛 index 를 보내 스피커를
    // 잠깐 되돌리는 레이스 방지
    var __navigating = false;
    window.__deckNavAnnounce = function (url) {
      __navigating = true;
      var t = __FILES.indexOf(url);
      if (t >= 0) __sync.send({ type: 'slide', index: t, fragment: 0, fragTotal: 0 });
    };
    __announce();
    __sync.on(function (msg) {
      if (msg.type === 'hello') { if (!__navigating) __announce(); return; }
