...
이 시스템은 나 혼자 쓰는 개인 자산에 머물지 않는다. 팔란티어가 자기 제품을 고객사 현장에 이식하듯, 내 디자인 색채를 다른 회사·다른 맥락에 입힐 수 있는 이식 가능한 제품으로 구축한다. 최종 형태는 아직 열려 있다 — 디자인 에이전트일 수도, Tailwind 같은 라이브러리일 수도, Figma 같은 제작 표면일 수도 있다. PPT·PDF·웹사이트를 포함한 모든 산출물 매체가 이식 대상이다.

> **성공 모습 · 움직이는 축 · 긴 arc** → `docs/vision/NORTH-STAR-detail.md`
> (2026-08-06 분리 — 상시 로드되는 자리에서 뺐다. 방향이 바뀔 때 그 파일과 위 본문을 함께 고친다.)
